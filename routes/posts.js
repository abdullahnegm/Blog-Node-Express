...
// Middlewares
const Auth = require("../middlewares/authMiddleware"); // For jwt tokens
...
postRoutes.get("/search", Paginate(Posts, false, true), async (req, res) => {
  console.log("search");

  return res.render("home", { posts: res.paginatedResults, user: req.user });
});
...
postRoutes.post("/", upload.single("avatar"), async (req, res, next) => {
  console.log("Here 5");
  try {
